	...
	function moveToItem(delta, p) {
	  ...
	  if (p) {
	    if (p.id == 'prev' || p.id == 'next') {
	      if (p.id == 'next')
	        movePagePrev(); //switched movePagePrev() and movePageNext
	      else
	        movePageNext();
	      return false;
	    }
	...
	function movePageNext() {
		if (getEl(config.nextPageSelector)[0]) {
	  	if (getEl(config.nextPageSelector)[0].href != null) {
	    	redirect(getEl(config.nextPageSelector)[0].href+'#'+config.bottomAnchor);
	    	disableHotKeys();
	    	return true;
			}
	  }
	...
	function movePagePrev() {
		if (getEl(config.prevPageSelector)[0]) {
	 	 if (getEl(config.prevPageSelector)[0].href != null) {
		    redirect(getEl(config.prevPageSelector)[0].href); //moved "+'#'+config.bottomAnchor" to movePageNext
		    disableHotKeys();
		    return true;
		  }
	  }
